#!/bin/bash
# ==============================================================================
# Complete Grafana & DevOps Monitoring Stack Installer for Amazon Linux 2023
# Installs: Grafana, Prometheus, and Node Exporter
# Auto-configures: Prometheus Scrape Targets + Grafana Data Source
...
echo "=========================================================================="
echo " Starting Full Grafana & Monitoring Stack Installation on Amazon Linux 2023"
echo "=========================================================================="
...
# ------------------------------------------------------------------------------
# 2. Stop Services If Running (Prevents "Text file busy" error on re-install)
# ------------------------------------------------------------------------------
echo "--> Stopping services if running to prevent file locks..."
...
sudo cp -f "${PROM_DIR}/prometheus" "${PROM_DIR}/promtool" /usr/local/bin/
sudo cp -rf "${PROM_DIR}/consoles" "${PROM_DIR}/console_libraries" /etc/prometheus/

# Configure prometheus.yml with Prometheus + Node Exporter jobs
...
# ------------------------------------------------------------------------------
# 4. Install & Configure Node Exporter
# ------------------------------------------------------------------------------
...
sudo cp -f "${NODE_DIR}/node_exporter" /usr/local/bin/
...
# ------------------------------------------------------------------------------
# 6. Auto-Provision Prometheus Data Source in Grafana
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 7. Auto-Import Grafana Dashboards (1860, 14282, 315, 3662)
# ------------------------------------------------------------------------------
...
download_dashboard() {
    ...
    sudo mv -f /tmp/dash.json "${TARGET_FILE}"
...
echo ""
echo "=========================================================================="
echo " 🎉 GRAFANA & DEVOPS MONITORING STACK INSTALLED SUCCESSFULLY! "
echo "=========================================================================="
